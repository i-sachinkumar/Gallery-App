...
class ImageLoader (context: Context, private val baseUrl: String?) :
    ...
    private fun extractFeatureFromJson(jsonResponse: String): List<String> {
        val list = ArrayList<String>()
        if(jsonResponse != "") {
            val jsonArray = JSONObject(jsonResponse).getJSONObject("photos").getJSONArray("photo")
            for (i in 1..jsonArray.length()) {
                val urlOfPhoto = jsonArray.getJSONObject(i-1).getString("url_s")
                list.add(urlOfPhoto)
            }
        }
        return list
